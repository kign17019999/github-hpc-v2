...
void do_wsp(int rank, int size){
    ...
    for(int i=0; i<num_init_path; i++){ 
        if(init_path_rank[i]==rank){ //check which initate that the procs should perform refer to init_rank
            ...
            branch_and_bound(path, init_bound[i], visited, init_level, rank); // compute branch_and_bound
        }
...
void branch_and_bound(int *path, int path_bound, int *visited, int level, int rank) {
    ...
    if (level == n) {   // check that the travelling is going to the end of path or not
	    if (path_bound < best_path_bound[rank]) {   //check the new path is better than the previous best or not
